...
void insert_tree(int i, int l, int h, int c) {
	...
	if (l < mid[i])
		insert_tree(i * 2 + 1, l, h, c);
	if (h > mid[i])
		insert_tree(i * 2 + 2, l, h, c);
}
...
void solve() {
	X.clear(); Y.clear();
	X.push_back(0); X.push_back(R); Y.push_back(0); Y.push_back(C);
	for (int i = 0; i < n; i++) {
