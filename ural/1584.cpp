...
void solve() {
  ...
  for (int x = 0; x < r / 2; x++) {
    for (int y = 0; y < c / 2; y++) {
      for (int j = 0; j < n; j++) {
        a[m][j] = 0;
        if (b[x][y] != chs[j]) a[m][j]++;
        if (b[r - 1 - x][y] != chs[j]) a[m][j]++;
        if (b[x][c - 1 - y] != chs[j]) a[m][j]++;
        if (b[r - 1 - x][c - 1 - y] != chs[j]) a[m][j]++;
      }
      m++;
    }
  }
  ...
  for (int x = 0; x < n; x++) {
    lx[x] = -oo;
    for (int y = 0; y < n; y++) lx[x] = max(lx[x], a[x][y]);
  }
  memset(match, -1, sizeof match);
  for (int x = 0; x < n; x++) {
    for (int y = 0; y < n; y++) slack[y] = oo;
    while (true) {
      ...
      int delta = oo;
      for (int y = 0; y < n; y++) {
        if (!vy[y] && slack[y] < delta) delta = slack[y];
      }
      for (int x = 0; x < n; x++) {
        if (vx[x]) lx[x] -= delta;
      }
