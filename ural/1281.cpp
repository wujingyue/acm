...
bool prev(const Point &a, const Point &b, const Point &c) {
	...
	return mul > o || (mul >= -o && dist(a.x, a.y, c.x, c.y) >= dist(b.x, b.y, c.x, c.y) - o);
}
...
struct by_slope {
	...
	bool operator()(const Point &a, const Point &b) {
		Real mul = cross(a.x - c.x, a.y - c.y, b.x - c.x, b.y - c.y);
		return mul > o || (mul >= -o && dist(a.x, a.y, c.x, c.y) < dist(b.x, b.y, c.x, c.y));
	}
};
...
void search(int i) {
	visit[i] = true;
	for (size_t k = 0; k < river[i].size(); ++k)
		p.push_back(river[i][k]);
	for (int j = 0; j < m; j++) {
		if (!visit[j] && a[i][j])
			search(j);
	}
...
Real convex_hull(vector<Point> &p) {
	...
	fprintf(stderr, "----------\n");
#endif
	int top = 2; t[0] = 0; t[1] = 1;
	for (int j = 2; j < n; j++) {
		while (top >= 2 && prev(p[j], p[t[top - 1]], p[t[top - 2]]))
			top--;
		t[top] = j;
		top++;
	}
