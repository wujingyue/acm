...
void solve() {
  ...
  for (int i = n - 1; i >= 0; i--) {
    ...
    if (a[i] == '.') num_dot[i]++;
    if (a[i] == 'X') num_cross[i]++;
  }
  memset(c, 0, sizeof c);
  for (int i = 0; i < n; i++) {
    if (i + len[0] - 1 >= n) continue;
    if (num_dot[i] - num_dot[i + len[0]] > 0) continue;
    if (num_cross[0] - num_cross[i] > 0) continue;
    c[i][0] = true;
  }
  for (int j = 1; j < k; j++) {
    for (int i = 0; i < n; i++) {
      if (i + len[j] - 1 >= n) continue;
      if (num_dot[i] - num_dot[i + len[j]] > 0) continue;
      for (int i1 = 0; i1 < i; i1++) {
        if (!c[i1][j - 1]) continue;
        if (i1 + len[j - 1] - 1 >= i - 1) continue;
        if (num_cross[i1 + len[j - 1]] - num_cross[i] > 0) continue;
        c[i][j] = true;
        break;
      }
    }
  ...
  memset(d, 0, sizeof d);
  for (int i = 0; i < n; i++) {
    if (i + len[k - 1] - 1 >= n) continue;
    if (num_dot[i] - num_dot[i + len[k - 1]] > 0) continue;
    if (num_cross[i + len[k - 1]] > 0) continue;
    d[i][k - 1] = true;
  }
  for (int j = k - 2; j >= 0; j--) {
    for (int i = 0; i < n; i++) {
      ...
      for (int i1 = i + 1; i1 < n; i1++) {
        if (!d[i1][j + 1]) continue;
        if (i + len[j] - 1 >= i1 - 1) continue;
        if (num_cross[i + len[j]] - num_cross[i1] > 0) continue;
        d[i][j] = true;
        break;
      }
    }
...
bool possible_cross(int i) {
  ...
  for (int i1 = 0; i1 < n; i1++) {
    for (int j = 0; j < k; j++) {
      if (i1 + len[j] - 1 >= i && i1 <= i && c[i1][j] && d[i1][j]) return true;
    }
  }
  return false;
}
...
bool possible_dot(int i) {
  ...
  for (int j = -1; j < k; j++) {
    ...
    if (j == -1)
      ...
      poss_left = false;
      for (int i1 = 0; i1 < n; i1++) {
        if (i1 + len[j] - 1 < i && c[i1][j] &&
            num_cross[i1 + len[j]] - num_cross[i] == 0) {
          poss_left = true;
          break;
        }
      }
    ...
    if (poss_left && poss_right) return true;
  }
  return false;
}
