...
bool search(int i, int sum, pair<int, int> *range, int n) {
	...
	for (stack[i] = range[i].first; stack[i] <= range[i].second; ++stack[i]) {
		if (search(i + 1, sum + stack[i], range, n))
			return true;
	}
	return false;
}
...
void print_percent_6(int a) {
	if (a < 0)
		...
		ostringstream oss;
		oss << a;
		for (size_t i = 0; i + oss.str().length() < 5; ++i)
			cout << ' ';
		cout << oss.str() << "%";
	}
}

void print_sheet(const string &sheet_name,
		const string &code1, const string &code2) {
	if (first)
		first = false;
	else
		cout << endl;
	...
	for (size_t x = 0; x < answers[i1].size(); ++x) {
		...
		print_6("");
		for (size_t y = 0; y <= answers[i2].size(); ++y)
			print_percent_6(b[x][y]);
		cout << endl;
		// percent y
		print_6("");
		for (size_t y = 0; y <= answers[i2].size(); ++y)
			print_percent_6(c[x][y]);
		cout << endl;
	}
	print_6("TOTAL");
	for (size_t y = 0; y <= answers[i2].size(); ++y)
