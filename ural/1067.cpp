...
struct Path {
  ...
  bool operator<(const Path &b) const {
    for (size_t i = 0; i < min(dirs.size(), b.dirs.size()); i++) {
      if (dirs[i] < b.dirs[i]) return true;
      if (dirs[i] > b.dirs[i]) return false;
    }
...
void print(int i1, int i2, int j) {
  for (int s = i1; s <= i2;) {
    int e = s + 1;
    while (e <= i2 && a[e].dirs[j] == a[s].dirs[j]) e++;
    print_spaces(j);
    printf("%s\n", a[s].dirs[j].c_str());
    while (s < e && (int)a[s].dirs.size() <= j + 1) s++;
    if (s < e) print(s, e - 1, j + 1);
    s = e;
  }
}
