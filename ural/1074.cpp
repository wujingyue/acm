...
void solve() {
	for (size_t i = 0; i < a.length(); i++) {
		if (a[i] == 'e' || a[i] == 'E' || a[i] == '+' || a[i] == '-' || (a[i] >= '0' && a[i] <= '9') || a[i] == '.')
			continue;
		ans = false;
		return;
	}
	size_t i = 0;
	while (i < a.length() && a[i] != 'e' && a[i] != 'E')
		i++;
	if (i >= a.length()) {
		p = 0;
		if (!parse_dec(a)) {
...
void output() {
	...
	if (dot <= 0) {
		...
		for (int i = 0; i < dot; i++) {
			if ((int)integral.size() >= MAX_N)
				break;
			integral += '0' + (i < MAX_N ? b[i] : '0');
		}
		for (int i = dot; i < MAX_N; i++)
			fractional += '0' + b[i];
	}
	while (integral.length() > 1 && integral[0] == '0')
		integral.erase(0, 1);
	string out;
	if (n == 0)
		out = integral;
