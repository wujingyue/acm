...
void add(BigInt &a, const BigInt &b) {
  ...
  for (int i = 0; i < a.len; i++) a.digit[i] += b.digit[i];
...
void input() {
  ...
  p = (int)forbid1.size();
  for (int i = 0; i < p; i++) forbid[i] = forbid1[i];
}
  ...
  for (int i = 0; i < p; i++) {
    for (int j = 0; j < (int)forbid[i].length(); j++) {
      for (int k = 0; k < n; k++) {
        ...
        if (!found) transfer[i][j][k] = make_pair(0, 0);
        // fprintf(stderr, "%d %d %d -> %d %d\n", i, j, k,
        // transfer[i][j][k].first, transfer[i][j][k].second);
      }
    }
  }
  for (int k = 0; k < m; k++) {
    for (int i = 0; i < p; i++) {
      for (int j = 0; j < (int)forbid[i].length(); j++) clear(c[k][i][j]);
    }
  }
  c[0][0][0].digit[0] = 1;
  for (int k = 0; k < m; k++) {
    for (int i = 0; i < p; i++) {
      for (int j = 0; j < (int)forbid[i].length(); j++) {
        if (c[k][i][j].len == 1 && c[k][i][j].digit[0] == 0) continue;
