...
int s[maxp][maxp],queue[maxp*maxnm],i,j,k,l,n,m,p,q,cl,op,total,now;
	...
	do
	{
		...
		for(i=1;i<=n;i++)
			if(trie[k].son[i]!=0)
			{
				...
				trie[trie[k].son[i]].danger=trie[trie[k].son[i]].danger || trie[trie[trie[k].son[i]].suffix].danger;
			}
			else 
