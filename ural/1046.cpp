...
void output() {
	a[0].x = x0; a[0].y = Y0;
	for (int i = 1; i < n; i++) {
		a[i].x = (a[i - 1].x - m[i - 1].x) * cos(al[i - 1]) - (a[i - 1].y - m[i - 1].y) * sin(al[i - 1]) + m[i - 1].x;
		a[i].y = (a[i - 1].x - m[i - 1].x) * sin(al[i - 1]) + (a[i - 1].y - m[i - 1].y) * cos(al[i - 1]) + m[i - 1].y;
	}
	for (int i = 0; i < n; i++)
		printf("%.2Lf %.2Lf\n", a[i].x, a[i].y);
}
