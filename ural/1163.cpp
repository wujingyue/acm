...
int get_removed(int i, Real th) {
	...
	for (int j = 0; j < n; j++) {
		if (i == j)
			continue;
		if (inside(th, a[i][j].first, a[i][j].second))
			removed |= (1 << j);
	}
	return removed;
}
...
void solve() {
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			...
			a[i][j].first = normalize(al - th); a[i][j].second = normalize(al + th);
		}
	}
	for (int i = 0; i < n; i++) {
		X.clear();
		...
		shoot[i].resize(unique(shoot[i].begin(), shoot[i].end()) - shoot[i].begin());
	}
	memset(c, -1, sizeof c);
	c[0][0] = 0; c[0][1] = 0;
	ans = get_c((1 << n) - 1, 0);
