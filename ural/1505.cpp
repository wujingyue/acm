...
void input() {
  ...
  for (int x = 1; x <= n; x++) {
    if (flow[x] < 0) add_edge(0, x, oo, -flow[x], 0);
  }
  n++;
}
...
void solve() {
  ...
  while (y != 0) {
    int x = p[y];
    a[x][w[y]].f++;
    a[x][w[y]].c = max(a[x][w[y]].c, a[x][w[y]].f);
    a[y][a[x][w[y]].g].f--;
    y = x;
  }
}
