...
Real len(const Point &a) { return sqrt((Real)a.x * a.x + (Real)a.y * a.y); }
...
bool can_merge(const Point &a, const Point &b) {
  ...
  return (long long)c.x * c.x + (long long)c.y * c.y <=
         (long long)a.x * a.x + (long long)a.y * a.y + (long long)b.x * b.x +
             (long long)b.y * b.y;
}
...
void solve() {
  for (int i = 0; i < 3; i++) zone[i].clear();
  for (int i = 0; i < n; i++) {
    zone[get_zone(p[i])].push_back(i);
  }
  m = n;
  for (int i = 0; i < max_m; i++) c[i].clear();
  while (true) {
    int j = 0;
    while (j < 3 && zone[j].size() < 2) j++;
    if (j == 3) break;
    int i1, i2;
    i1 = zone[j].back();
    zone[j].pop_back();
