...
int Lower_Bound(int i, int j, int k) {
	...
	while (low <= high) {
		int mid = (low + high) / 2;
		if (q[mid].i == i && q[mid].j == j && q[mid].k == k)
			return mid;
		if (i < q[mid].i || (i == q[mid].i &&
					(j < q[mid].j || (j == q[mid].j && k < q[mid].k))))
			high = mid - 1;
		else
			low = mid + 1;
	}
...
void init() {
	scanf("%d %d %d", &n, &h, &m);
	memset(q, -1, sizeof q);
	head = 0; tail = 1; q[0].i = m; q[0].j = h; q[0].k = n;
	memset(visited, 0, sizeof visited);
	while (head < tail) {
		int i = q[head].i, j = q[head].j, k = q[head].k;
		int i1, j1, k1;
		if (i > 1 && j > 1 && i <= k) {
			i1 = i - 1; j1 = j - 1; k1 = k - i;
			if (!is_visited(i1, k1)) {
				set_visited(i1, k1);
				my_assert(tail < max_t);
				q[tail].i = i1; q[tail].j = j1; q[tail].k = k1; ++tail;
			}
		}
		if (j > 1 && i <= k) {
			i1 = i + 1; j1 = j - 1; k1 = k - i;
