...
void get_inter(Real a1, Real b1, Real c1, Real a2, Real b2, Real c2, Real &x,
               Real &y) {
  x = multi(b1, c1, b2, c2) / multi(a1, b1, a2, b2);
  y = multi(c1, a1, c2, a2) / multi(a1, b1, a2, b2);
}

void get_inter2(Real a1, Real b1, Real c1, Real a2, Real b2, Real c2, Real &x,
                Real &y) {
...
Real area(const vector<Point> &p) {
  ...
  for (size_t i = 1; i + 1 < p.size(); i++)
    res += multi(p[i].x - p[0].x, p[i].y - p[0].y, p[i + 1].x - p[0].x,
                 p[i + 1].y - p[0].y);
  return abs(res) / 2;
}
...
void solve() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = i + 1; j < n; j++) {
      ...
      a2 = p[j + 1].y - p[j].y;
      b2 = p[j].x - p[j + 1].x;
      if (a1 * a1 + b1 * b1 <= o || a2 * a2 + b2 * b2 <= o) continue;
  ...
  for (size_t k = 0; k < cand.size(); k++) {
    ...
    while (h - l > 1e-6) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if ((c1 <= c + o && c2 > c + o) || (c1 > c + o && c2 <= c + o)) {
          Real x, y;
          get_inter2(p[i + 1].y - p[i].y, p[i].x - p[i + 1].x,
                     -multi(p[i].x, p[i].y, p[i + 1].x, p[i + 1].y), a, b, -c,
                     x, y);
    ...
    for (int i = 0; i < n; i++) {
      ...
      if ((c1 <= l + o && c2 > l + o) || (c1 > l + o && c2 <= l + o)) {
        Real x, y;
        get_inter2(p[i + 1].y - p[i].y, p[i].x - p[i + 1].x,
                   -multi(p[i].x, p[i].y, p[i + 1].x, p[i + 1].y), a, b, -l, x,
                   y);
    ...
    Real d =
        sqrt(sqr(inters[0].x - inters[1].x) + sqr(inters[0].y - inters[1].y));
    if (d < ans) ans = d;
  }
}
