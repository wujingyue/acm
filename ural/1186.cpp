...
vector<string> convert(const string &a) {
  ...
  for (size_t i = 0; i < a.length();) {
    ...
    if (a[i] == '+' || a[i] == '(' || a[i] == ')') {
      ...
      assert(isupper(a[i]));
      size_t j = i + 1;
      while (j < a.length() && islower(a[j])) j++;
      tokens.push_back(a.substr(i, j - i));
      i = j;
    }
  }
...
void calc(vector<Result> &opr, vector<string> &opt) {
  ...
  if (opt.back() == "+") {
    ...
    for (map<string, int>::iterator it = opr[m - 1].res.begin();
         it != opr[m - 1].res.end(); ++it)
    ...
    if (opr[m - 2].num > 0 && opr[m - 1].num > 0)
      ...
      if (opr[m - 2].num > 0) swap(opr[m - 2], opr[m - 1]);
      for (map<string, int>::iterator it = opr[m - 2].res.begin();
           it != opr[m - 2].res.end(); ++it)
