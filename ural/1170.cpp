...
void input() {
  scanf("%d", &n);
  for (int i = 0; i < n; i++) {
    scanf("%Lf %Lf %Lf %Lf %Lf", &r[i].x1, &r[i].y1, &r[i].x2, &r[i].y2,
          &r[i].c);
  }
  scanf("%Lf %Lf", &c0, &L);
}
...
bool get_inter(Real x1, Real y1, Real x2, Real y2, Real x3, Real y3, Real x4,
               Real y4, Real &x, Real &y) {
  if (cross(x3, y3, x2, y2) * cross(x4, y4, x2, y2) <= o) {
    ...
    a1 = y3 - y4;
    b1 = x4 - x3;
    c1 = cross(x3, y3, x4, y4);
    a2 = y1 - y2;
    b2 = x2 - x1;
    c2 = cross(x1, y1, x2, y2);
...
Real get_time(Real x0, Real y0) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    Real x1, y1, x2, y2;
    x1 = inters.front().first;
    y1 = inters.front().second;
    x2 = inters.back().first;
    y2 = inters.back().second;
