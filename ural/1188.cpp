...
bool operator<(const Board &a, const Board &b) { return a.y < b.y; }
...
void input() {
  ...
  scanf("%d", &n);
  for (int i = 0; i < n; i++) {
    scanf("%d %d %d %d %d", &b[i].y, &b[i].x1, &b[i].x2, &b[i].peg1,
          &b[i].peg2);
    b[i].x2 += b[i].x1;
    b[i].peg1 += b[i].x1;
    b[i].peg2 += b[i].x1;
...
void solve() {
  sort(b, b + n);
  for (int i = 0; i < n; i++) {
    if (b[i].x2 - b[i].x1 < W || b[i].y + H > R) continue;
    for (int x = 0; x + b[i].x2 - b[i].x1 <= C; ++x) {
      int x1 = x, x2 = x + b[i].x2 - b[i].x1;
      if (x1 <= b[i].peg1 && b[i].peg1 <= x2 && x1 <= b[i].peg2 &&
          b[i].peg2 <= x2 && b[i].peg1 * 2 <= x1 + x2 &&
          x1 + x2 <= b[i].peg2 * 2) {
        bot[i][x] = 0;
        continue;
      }
    ...
    s.clear();
    for (int x = 0; x + W <= C; ++x) {
      if (x + b[i].x2 - b[i].x1 <= C) s.insert(bot[i][x]);
      if (x + W - (b[i].x2 - b[i].x1) - 1 >= 0) {
        multiset<int>::iterator it =
            s.find(bot[i][x + W - (b[i].x2 - b[i].x1) - 1]);
        assert(it != s.end());
        s.erase(it);
      }
      assert(!s.empty());
  ...
  for (int i = 0; i < n; i++) {
    for (int x = 0; x + W <= C; ++x) {
      ...
      if (b[i].peg1 <= x1 && b[i].peg2 >= x2) {
        ...
        if (max_l < b[i].x2 - b[i].x1)
          aside[i][x] =
              make_pair(max_l == 0 ? 2 : 1, b[i].x2 - b[i].x1 - max_l);
        else
  ...
  for (int x = 0; x + W <= C; x++) {
    ...
    int j = 0;
    for (int i = 0; i < n; i++) {
      while (j < n && b[j].y < b[i].y + H) j++;
      if (b[i].x2 - b[i].x1 >= W && b[i].y + H <= R) {
        pair<int, int> tmp =
            make_pair(sum_aside[j - 1].first - sum_aside[i].first,
        ...
        if (tmp < ans) {
          ...
          cerr << "x = " << x << endl;
          cerr << "support = " << support[i][x] << "; " << "bot = " << bot[i][x]
          << endl; for (int k = i; k < j; k++) cerr << b[k].y << ' ' << b[k].x1
          << ' ' << b[k].x2 << ' ' << b[k].peg1 << ' ' << b[k].peg2 << endl;
          */
          ans = tmp;
        }
      }
